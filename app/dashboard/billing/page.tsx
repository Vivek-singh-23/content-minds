...
function billing() {
  ...
  const handleUpgrade = () => {
    if (!process.env.NEXT_PUBLIC_RAZORPAY_KEY_ID) {
      // Show toast notification if Razorpay key is not available
      toast.error("Owner haven't added his razorpay ID");
      return;
    }
    // Logic to start the upgrade (e.g., create subscription or payment flow)
    setLoading(true);

    setTimeout(()=>{
      setLoading(false)
    }, 2000)
    // Simulate the API call or subscription creation here
  };
        ...
        px-12 py-3 text-center text-sm font-medium bg-gray-500 text-white
          ...
          <div className="rounded-2xl bg-white border border-gray-200 p-6 shadow-sm sm:px-8 lg:p-12">
            ...
            >
              ...
              {userSubscription ? "Active Plan" : "Upgrade"}
            </Button>
          </div>
        </div>
      </div>
